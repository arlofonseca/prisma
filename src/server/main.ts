    ...
    try {
      ...
      for (const user of data) {
        exports.chat.addMessage(
          source,
          `User ID: ^#5e81ac${user.userId} ^#ffffff| Username: ^#5e81ac${user.username ?? 'N/A'} ^#ffffff| License2: ^#5e81ac${user.license2} ^#ffffff| Steam: ^#5e81ac${user.steam ?? 'N/A'} ^#ffffff| FiveM: ^#5e81ac${user.fivem ?? 'N/A'} ^#ffffff| Discord: ^#5e81ac${user.discord ?? 'N/A'}`,
        );
      }
    } catch (error) {
      ...
      exports.chat.addMessage(
        source,
        '^#d73232ERROR ^#ffffffAn error occurred while trying to fetch all user data.',
      );
    }
  },
  {
...
addCommand(
  ...
  async (source: number, args: { stateId: string }): Promise<void> => {
    ...
    try {
      ...
      const name: string = character.lastName
        ? `${character.firstName} ${character.lastName}`
        : character.firstName;
      exports.chat.addMessage(
        source,
        `^#5e81ac[ADMIN] ^#ffffffName: ^#5e81ac${name} ^#ffffff| Gender: ^#5e81ac${character.gender} ^#ffffff| Date of Birth: ^#5e81ac${character.dateOfBirth.toISOString().split('T')[0]} ^#ffffff| Phone Number: ^#5e81ac${character.phoneNumber || 'N/A'} ^#ffffff| Health: ^#5e81ac${character.health !== null ? character.health : 'N/A'} ^#ffffff| Armour: ^#5e81ac${character.armour !== null ? character.armour : 'N/A'} ^#ffffff| Statuses: ^#5e81ac${character.statuses || 'N/A'}`,
      );
    } catch (error) {
      console.error('/viewchar:', error);
      exports.chat.addMessage(
        source,
        '^#d73232ERROR ^#ffffffAn error occurred while trying to view character.',
      );
    }
  },
  {
...
addCommand(
  ...
  async (
    ...
    try {
      ...
      exports.chat.addMessage(
        source,
        `^#5e81acSuccessfully changed character's name to ^#ffffff${firstName} ${lastName}`,
      );
    } catch (error) {
      console.error('/updatechar:', error);
      exports.chat.addMessage(
        source,
        '^#d73232ERROR ^#ffffffAn error occurred while trying to update the name of the character.',
      );
    }
  },
  {
...
addCommand(
  ...
  async (source: number): Promise<void> => {
    ...
    try {
      ...
      exports.chat.addMessage(
        source,
        '^#d73232ERROR ^#ffffffAn error occurred while trying to count characters.',
      );
    }
  },
  {
  ...
  async (source: number, args: { limit?: number }): Promise<void> => {
    ...
    try {
      ...
      if (count === 0) {
        ...
        exports.chat.addMessage(
          source,
          `^#5e81ac[ADMIN] ^#ffffffDeleted ^#5e81ac${count} ^#ffffffinactive characters who haven't been active for more than ${limit} days.`,
        );
      }
    } catch (error) {
      ...
      exports.chat.addMessage(
        source,
        '^#d73232ERROR ^#ffffffAn error occurred while trying to delete inactive characters.',
      );
    }
  },
  {
...
addCommand(
  ...
  async (source: number, args: { firstName: string }): Promise<void> => {
    ...
    try {
      ...
      exports.chat.addMessage(
        source,
        '^#5e81ac--------- ^#ffffffCharacter Results ^#5e81ac---------',
      );
      for (const character of characters) {
        const name: string = character.lastName
          ? `${character.firstName} ${character.lastName}`
      ...
      exports.chat.addMessage(
        source,
        '^#d73232ERROR ^#ffffffAn error occurred while trying to search for characters.',
      );
    }
  },
  {
